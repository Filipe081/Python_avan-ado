...
def extract_movies(soup, max_threads):
...
def main(max_threads):
    ...
    extract_movies(soup, max_threads)

    end_time = time.time()
    print(f'Total time taken with {max_threads} threads: {end_time - start_time} seconds')

if __name__ == '__main__':
    for max_threads in [1, 5, 10, 20, 50]:
        print(f"\nRunning with {max_threads} threads")
        main(max_threads)
